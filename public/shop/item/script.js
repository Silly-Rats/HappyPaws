...
function cartAlert(btn, price, name, qty, select, obj) {
    ...
    prdName.textContent = name.textContent;
    prdType.textContent = select.value;
    orderQty.textContent = `x${qty.textContent}`;
    let totalPrice = parseFloat(price.textContent) * (+(qty.textContent));
    orderPrc.textContent = totalPrice.toFixed(2) + '$';

    const index = select.selectedIndex;
    let newQty = cartItemsMap.get(obj.types[index].id);
    if (newQty === undefined) {
        newQty = 0;
    }
    newQty += +qty.textContent;
    cartItemsMap.set(obj.types[index].id, newQty);
